...
**Independent occurrences**: N/A — this promotion's evidentiary basis is a captain ruling already recorded (`candidates/2026-07-18-policies-vs-requirements-split.md`, status: ratified), not the ≥2-independent-validations bar this pipeline normally requires for validator-discovered patterns. Flagged honestly per the sweep's own anti-goal against inflating evidence: this is a different evidence class (direct human ruling) than the repeated-failure pattern most promotions in this pipeline document.
